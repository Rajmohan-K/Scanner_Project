...
import { Pause, Play, RotateCcw, SlidersHorizontal, Square, Timer, Zap } from 'lucide-react';
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        toast?.push('Unable to load scans from backend', 'error');
      } finally {
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    async function pollActiveScan() {
      try {
        ...
        if (!cancelled) toast?.push('Unable to read active scan status', 'error');
      }
    }

    pollActiveScan();
    const timer = window.setInterval(pollActiveScan, 1000);
    return () => {
      cancelled = true;
  ...
  return (
    <main>
      <PageHero
        ...
        actions={<><button className="btn-primary" onClick={handleStart}><Play size={16} /> Start Scan</button><button className="btn-secondary"><Timer size={16} /> Schedule</button></>}
        metrics={[
