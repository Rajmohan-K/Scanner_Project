...
export default function GrowwIntradayPage() {
  ...
  const [priorityRows, setPriorityRows] = useState<any[]>([]);
  ...
  useEffect(() => {
    function sync() {
      ...
      if (latest.rows.length) {
        ...
        setPriorityRows(latest.priorityRows || []);
  ...
  async function runGrowwScan() {
    ...
    try {
      ...
      setPriorityRows(saved.priorityRows || []);
  ...
  return (
    <main>
      ...
      <TerminalPanel eyebrow="Automation" title="Continuous Groww Intraday Checker">
        <div className="form-grid">
          ...
          <label className="field">
            ...
            <select value={settings.priorityLimit || 5} onChange={(event) => updateSettings({ priorityLimit: Number(event.target.value) })}>
          ...
          <label className="field">
            ...
            <select value={settings.priorityMinProfitPct || 3} onChange={(event) => updateSettings({ priorityMinProfitPct: Number(event.target.value) })}>
        ...
        <p className="small">When enabled, the app keeps checking Groww and refreshes the Groww results cache only. Intraday scanner input changes only when you explicitly send or pull the list.</p>
      </TerminalPanel>
      ...
      <TerminalPanel eyebrow="Filtered Output" title="Shared Intraday Engine Results">
