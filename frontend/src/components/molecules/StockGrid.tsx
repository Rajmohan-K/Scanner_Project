...
function StockGridComponent({
  ...
  const sortedItems = useMemo(() => {
    ...
    return copy;
  }, [localItems, sortDirection, sortKey]);
    ...
    <div className={`stock-list-table stock-list-table--${density}`} role="table" aria-label="Live stock results">
      <div className="data-grid-toolbar">
        <div>
          ...
          <span>{sortedItems.length} analyzed stocks</span>
        </div>
      ...
      {selectedStock && (
        <div className="selected-stock-detail">
          <div>
            <span>Selected Stock Reason</span>
            <strong>{selectedStock.symbol || selectedStock.stock}</strong>
          </div>
          <p>{fullReason(selectedStock)}</p>
          <button className="icon-button" type="button" title="Close details" onClick={() => setSelectedStock(null)}>×</button>
        </div>
      )}
