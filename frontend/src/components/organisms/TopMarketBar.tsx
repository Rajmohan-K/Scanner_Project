...
export default function TopMarketBar() {
  ...
  const realtime = useRealtimeSnapshot(3000);
  ...
  useEffect(() => {
    ...
    loadIndices();
    const timer = window.setInterval(() => loadIndices(true), 3000);
  ...
  return (
    <div className="top-market-bar">
      ...
      <div className="top-ticker-strip">
        ...
        <div className={`realtime-health-chip ${realtimeStatus.toLowerCase()}`}>
          <span>Realtime</span>
          <strong>{realtimeStatus}{typeof realtimeAge === 'number' ? ` / ${realtimeAge}s` : ''}</strong>
        </div>
      </div>
